...
export const encryptArrayBuffer = async (
    ...
    console.log("上传: " + arrBuf.byteLength)
    ...
    while (startPosition < arrBuf.byteLength) {
        const endPosition = Math.min(startPosition + CHUNK_SIZE, arrBuf.byteLength);
...
export const decryptArrayBuffer = async (
    ...
    while (startPosition < cipherText.byteLength) {
        ...
        const endPosition = Math.min(startPosition + CHUNK_SIZE + 16, cipherText.byteLength);
